...
* The local identifier for a library must be unqualified and unique within the artifact.
...
NOTE: Implementations are not required to support unit conversion, and may return null, even if the conversion is valid. Implementations that do support unit conversion must do so according to the conversion behavior specified by UCUM.
...
Note the use of the `"Mother"` expression within the retrieve: `["Mother" -> "Observation": "Estimated Due Date"]`. This syntax (`->`) indicates that the retrieve should be performed in the context returned by the `"Mother"` expression. The `"Mother"` expression in this case will be evaluated in the `"Patient"` context, and result in a `RelatedPerson` from the infant's record with the relationship type of `"Mother Relationship"`. The `RelatedPerson` will then be used as the context for the retrieve.
