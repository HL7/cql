...
Retrieve : Expression
  ...
  0..1 --> codeProperty <6>
  ¦
  0..1 --> valueSetProperty <7>
  ¦
  0..1 --> dateProperty <8>
  ¦
  0..1 --> dateLowProperty <9>
  ¦
  0..1 --> dateHighProperty <10>
  ¦
  0..1 --> scope
----
...
==== dataType Attribute
<3> The dataType attribute specifies the type of data being requested.

==== templateId Attribute
<4> For data models that support templates (also called profiles), The templateId attribute specifies an optional template to be used. If specified, the retrieve is defined to return only objects that conform to the template.

==== idProperty Attribute
<5> The idProperty attribute specifies which property of the model contains the Id for the clinical statement.

==== codeProperty Attribute
<6> The codeProperty attribute optionally specifies which property of the model contains the Code or Codes for the clinical statement.
...
==== valueSetProperty Attribute
<7> The valueSetProperty attribute optionally specifies which property of the model contains a value set identifier that can be used as an alternative mechanism for matching the value set of the retrieve, in the case when no code is specified in the source data.
...
==== dateProperty Attribute
<8> The dateProperty attribute optionally specifies which property of the model contains the clinically relevant date for the clinical statement.
...
==== dateLowProperty Attribute
<9> The dateLowProperty attribute optionally specifies which property of the model contains the low component of the clinically relevant date for the clinical statement.
...
==== dateHighProperty Attribute
<10> The dateHighProperty attribute optionally specifies which property of the model contains the high component of the clinically relevant date for the clinical statement.
...
==== scope Attribute
<11> The scope attribute optionally specifies a name for the scope for this operator that can be used within expressions to reference the current iteration element.
...
The per argument determines the precision at which the collapse is computed and must be a quantity-valued expression compatible with the interval point type. For numeric intervals, this means a default unit ('1'), for Date-, DateTime-, and Time-valued intervals, this means a temporal duration.
...
The per argument determines the size of the resulting intervals and must be a quantity-valued expression compatible with the interval point type. For numeric intervals, this means a default unit ('1'), for Date-, DateTime-, or Time-valued intervals, this means a temporal duration.
...
The OverlapsBefore operator returns true if the first interval starts before and overlaps the second. In other words, if the first interval properly contains the starting point of the second interval.
...
The OverlapsAfter operator returns true if the first interval overlaps and ends after the second. In other words, if the first interval properly contains the ending point of the second interval.
