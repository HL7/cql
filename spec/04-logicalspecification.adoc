...
For more information on the primitive types, see the <<09-b-cqlreference.adoc#types-2,Types>> section in the link:09-b-cqlreference.html[CQL Reference].
...
Library : Element
  ¦
  1..1 --> identifier : VersionedIdentifier
  ¦
  1..1 --> schemaIdentifier : VersionedIdentifier
  ¦
  0..1 --> usings
  ¦          ¦
  ¦          1..* --> def : UsingDef
...
VersionedIdentifier is composed of two parts:

. an identifier which identifies the set of all versions of a given resource.
. the actual version of the instance of interest in this set.

The VersionedIdentifier therefore points to an individual 'versioned' instance of a resource such as the third version of a library.
...
The ExpressionDef type defines an expression and an associated name that can be referenced by any expression in the artifact. The name must be unique within the artifact. The context attribute specifies the scope of the execution and is used by the environment to determine when and how to evaluate the expression.
...
Refer to the Equal section in the <<equal,Comparison Operators>>.

=== Equivalent

Refer to the Equivalent section in the <<equivalent,Comparison Operators>>.
...
Refer to the Not Equal section in the <<not-equal,Comparison Operators>>.
...
Refer to the Add section in the <<add,Arithmetic Operators>>.

=== After

Refer to the After section in the <<after-1,Interval Operators>>.

=== Before

Refer to the Before section in the <<before-1,Interval Operators>>.
...
The SameAs operator is defined for Date, DateTime, and Time values, as well as date/time-based intervals.
...
For the Interval overloads, the SameAs operator returns true if the intervals start and end at the same value.
...
The SameOrBefore operator is defined for Date, DateTime, and Time values, as well as date/time-based intervals.
...
The SameOrAfter operator is defined for Date, DateTime, and Time values, as well as date/time-based intervals.
...
Refer to the Subtract section in the <<subtract,Arithmetic Operators>>.
...
For the Date, DateTime, and Time overloads, the After operator returns true if the first datetime is after the second datetime at the specified level of precision. As with all date/time calculations, comparisons are performed respecting the timezone offset.
...
For the Date, DateTime, and Time overloads, the Before operator returns true if the first datetime is before the second datetime at the specified level of precision. As with all date/time calculations, comparisons are performed respecting the timezone offset.
...
The per argument determines the precision at which the collapse is computed and must be a quantity-valued expression compatible with the interval point type. For numeric intervals, this means a default unit ('1'), dor date/time-valued intervals, this means a temporal duration.
...
For the List, T overload, this operator returns true if the given element is in the list, using equivalence semantics. If the list-valued argument is null, the result is null.
...
For the list overload, this operator returns a list with the elements that appear in the first operand, that do not appear in the second operand, using equivalence semantics. The operator is defined with set semantics, meaning that each element will appear in the result at most once, and that there is no expectation that the order of the inputs will be preserved in the results. If the left argument is null, the result is null. If the right argument is null, the result is the left argument.
...
The Expand operator returns the set of unit intervals for all the ranges present in the given list of intervals.
...
For the T, List overload, this operator returns true if the given element is in the given list, using equivalence semantics. If the list-valued argument is null, the result is null.
...
For the list overload, this operator returns a list with the elements that appear in both lists, using equivalence semantics. The operator is defined with set semantics, meaning that each element will appear in the result at most once, and that there is no expectation that the order of the inputs will be preserved in the results.
...
Refer to the Contains section in the <<contains,Interval Operators>>.
...
Refer to the Except section in the <<except,Interval Operators>>.
...
Refer to the In section in the <<in,Interval Operators>>.

=== Includes

Refer to the Includes section in the <<includes,Interval Operators>>.

=== IncludedIn

Refer to the IncludedIn section in the <<included-in,Interval Operators>>.
...
Refer to the Intersect section in the <<intersect,Interval Operators>>.
...
Refer to the ProperContains section in the <<proper-contains,Interval Operators>>.

=== ProperIn

Refer to the ProperIn section in the <<proper-in,Interval Operators>>.

=== ProperIncludedIn

Refer to the ProperIncludedIn section in the <<proper-included-in,Interval Operators>>.

=== ProperIncludes

Refer to the ProperIncludes section in the <<proper-includes,Interval Operators>>.
...
Refer to the Union section in the <<union,Interval Operators>>.
...
This operator supports testing for conversion between String and each of Boolean, Integer, Decimal, Quantity, Ratio, Date, DateTime, and Time, as well as conversion from Integer to Decimal, Integer or Decimal to Quantity, and from Code to Concept.
...
This operator supports conversion between String and each of Boolean, Integer, Decimal, Quantity, Date, DateTime, and Time, as well as conversion from Integer to Decimal and from Code to Concept.
...
The ConvertsToBoolean operator returns true if the value of its argument can be converted to a Boolean value. The operator accepts "true", "t", "yes", "y", and "1" as string representations of true, and "false", "f", "no", "n", and "0" as string representations of false, ignoring case.
...
The ConvertsToDate operator returns true if the value of its argument can be converted to a Date value. The operator expects the string to be formatted using the ISO-8601 date representation:
...
The ConvertsToTime operator returns true if the value of its argument can be converted to a Time value. The operator expects the string to be formatted using ISO-8601 time representation:
