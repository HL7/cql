...
For Date values, precision must be one of Year, Month, or Day. Note specifically that since there is variability how weeks are counted, Week precision is not supported, and will result in an error.
...
The ToBoolean operator returns true if the value of its argument can be converted to a Boolean value.
...
The ToDate operator returns true if the value of its argument can be converted to a Date value.
