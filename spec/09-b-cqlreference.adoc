...
When this operator is called with both [.id]#Date# and [.id]#DateTime# inputs, the [.id]#Date# values will be implicitly converted to [.id]#DateTime# values with [.kw]#null# time components.
...
The following examples illustrate the behavior of the [.kw]#on or after# operator:

[source,cql]
----
define OnOrAfterIsTrue: Interval[6, 10] on or after Interval[0, 5]
define OnOrAfterIsFalse: 6 after or on Interval[0, 7]
define OnOrAfterIsNull: Interval[6, 10] on or after (null as Integer)
----
...
The following examples illustrate the behavior of the [.kw]#on or before# operator:

[source,cql]
----
define OnOrBeforeIsTrue: Interval[0, 5] on or before Interval[5, 10]
define OnOrBeforeIsFalse: 1 before or on Interval[0, 7]
define OnOrBeforeIsNull: Interval[6, 10] on or before (null as Integer)
----
...
define ExistsIsNull: exists null
----
...
define InIsAlsoTrue: null in { 1, 3, 5, null }
...
define IncludesIsAlsoTrue: { 1, 3, 5, null } includes null
define IncludesIsFalse: { 1, 3 } includes { 1, 3, 5 }
define IncludesIsAlsoFalse: null includes { 1, 3, 5 }
----
...
define IncludedInIsAlsoTrue: null included in { 1, 3, 5 }
define IncludedInIsFalse: { 1, 3, 5 } included in { 1, 3 }
define IncludedInIsAlsoFalse: { 1, 3, 5, null } included in null
----
...
Note that for [.id]#AgeInYears#, the [.id]#birthDate# is specified as a [.id]#Date# and [.id]#Today()# is used to obtain the current date; whereas with the other precisions, [.id]#birthDate# is specified as a [.id]#DateTime#, and [.id]#Now()# is used to obtain the current date/time.
...
[source,cql]
----
parameter Numerator Integer
parameter Denominator Integer
