...
[source,cql]
----
define IsValid: ToDecimal('-1')
define IsNull: ToDecimal('one')
----
...
The _equivalence_ ([.sym]#~#) operator returns [.kw]#true# if the arguments are equivalent in value, or if they are both [.kw]#null#; and [.kw]#false# otherwise.
...
The following examples illustrate the behavior of the _equivalence_ operator:
