...
Intuitively, what this means is that the number of days between January 15^th^, 2014 and some date in February, 2014, is no less than 17 days, but no more than 44. And so, when a duration calculation results in an _uncertainty_, a duration interval is returned:

[source,cql]
----
define DurationInDays: days between @2017-08-07T17:00 and @2017-08-14 // [6, 7]
----
...
By incorporating this information into an uncertainty, CQL can support the intuitively expected semantics when performing timing comparisons. For example:
...
Uncertainties may also result in an interval consisting of durations between datetime values. For example:
...
Because the datetime values are both only specified to the month (from January of 2012 to February of 2012), it is unclear what duration is desired. And so, the result is an uncertainty interval beginning with the duration of the 31st of January, 2012 to the 1st of February, 2012 (the shortest possible duration), and ending with the duration of the 1st of January, 2012 to the 29th of February, 2012 (the longest possible duration).
...
Note that as with the before and after transformations, a null test is required here because if B is null, then the constructed interval would be incorrectly interpreted as continuing to the beggining or end of time (depending on whether the null appears as the starting or ending boundary of the interval).
