...
|Calendar day |Time interval starting at midnight and ending at the next midnight, the latter being also the starting instant of the next calendar day. a|
...
|Calendar year |Cyclic time interval in a calendar which is required for one revolution of the Earth around the Sun and approximated to an integral number of calendar days. a|
...
Note carefully that these semantics introduce some asymmetries into the comparison operators. In particular, _A = B or A < B_ is _not_ equivalent to _A \<= B_ because of the uncertainty.
...
_some value between 17 and 44_ + _some value between 5 and 10_
