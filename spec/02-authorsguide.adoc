...
Note that throughout the discussion, readers may wish to refer to link:09-b-cqlreference.html[Appendix B – CQL Reference] for more detailed discussion of particular concepts.
...
code "Acute Viral Pharyngitis Code":
  '195662009' from "SNOMED:2014" display 'Acute viral pharyngitis (disorder)'
concept "Acute Pharyngitis Concepts":
  { "Acute Pharyngitis Code", "Acute Viral Pharyngitis Code" } display 'Acute Pharyngitis Concepts'

define "Get Condition from Code Declaration":
  [Condition: severity in "Acute Pharyngitis Code"]

define "Get Condition from Concept Declaration":
  [Condition: severity in "Acute Pharyngitis Concepts"]

define "Get Condition from CodeSystem Declaration":
  [Condition: severity in "SNOMED:2014"]
----

The [.id]#"Get Condition from Code Declaration"# expression returns conditions for the patient where the severity is equal to the [.id]#"Acute Pharyngitis Code"# code. The [.id]#"Get Condition from Concept Declaration"# expression returns conditions for the patient where the severity is contained within the [.id]#"Acute Pharyngitis Concepts"# concept. The [.id]#"Get Condition from CodeSystem Declaration"# expression returns conditions for the patient where the severity is contained within the [.id]#"SNOMED:2014"# code system.
...
By default, queries always return a distinct list of results.  In other words, any duplicate values are suppressed from the results.
...
The following example illustrates the code declaration:
...
The example specifies the code [.lit]#'66071002'# from the previously defined [.id]#"SNOMED-CT:2014"# codesystem, which specifies both the [.id]#system# and [.id]#version# of the resulting code. Note that the [.kw]#display# clause is optional.
...
concept "Blood Pressure Concepts": { "Blood Pressure", "Systolic Blood Pressure", "Diastolic Blood Pressure" } display 'Blood Pressure Concepts'
----
...
|*~* |Equivalent |Returns true if the arguments are equivalent in value, or are both unknown. Returns null if either or both arguments are null
|*!~* |Inequivalent |Returns true if the arguments are not equivalent. Returns null if either or both arguments are null
|===========================================================================================================================================
...
Comparison, like equality, when no precision is specified, is performed to the unit with the most precision of either input. For example:
----
Date@2015-01-03 = DateTime@2015-01-03
----
This expression returns true, since the DateTime value is only specified to the day.

However, each operator also allows a precision specifier to be used. For example:

[source,cql]
----
Date(2014) same year as Date(2014, 7, 11)
Date(2014, 7) same month as Date(2014, 7, 11)
Date(2014, 7, 11) same day as DateTime(2014, 7, 11, 14, 0, 0)
----
...
Date(2014, 7, 15) same day or after DateTime(2014, 7, 11, 14, 0, 0)
----
...
Date(2014, 7, 15) after day of DateTime(2014, 7, 11, 14, 0, 0)
----
...
For calculations involving weeks, Sunday is considered the first of the week for the purposes of determining boundaries, and the duration of a week is always considered to be 7 days.
...
Note that the distinct operator uses the notion of equivalence ([.sym]#~#) to detect duplicates. Because equivalence is defined for all types, this means that [.kw]#distinct# can be used on lists with elements of any type. In particular, duplicates can be eliminated from lists of tuples, and the operation will use tuple equivalence (i.e. tuples are equal if they have the same type and an equivalent value (or no value) for each element of the same name).
...
These statements define the [.id]#InPharyngitis# expression as true if the Code-valued expression [.id]#SomeCodeValue# is in the [.id]#"Acute Pharyngitis"# valueset. Note that valueset membership is based strictly on the definition of equivalence (i.e. two codes are the same if they have an equivalent value for the code, system, and version elements). CQL explicitly forbids the notion of _terminological_ _equivalence_ among codes being used in this context.
...
|*AgeInMonthsAt(X)* |Determines the age of the patient in months as of the datetime X
|*AgeInMonths()* |Determines the age of the patient in months as of now. Equivalent to AgeInMonthsAt(Now())
|*AgeInWeeksAt(X)* |Determines the age of the patient in weeks as of the datetime X
|*AgeInWeeks()* |Determines the age of the patient in weeks as of now. Equivalent to AgeInWeeksAt(Now())
|*AgeInDaysAt(X)* |Determines the age of the patient in days as of the datetime X
...
|*CalculateAgeInMonthsAt(D, X)* |Determines the age of a person with birthdate D in months as of the datetime X
|*CalculateAgeInMonths(D)* |Determines the age of a person with birthdate D in months as of now. Equivalent to CalculateAgeInMonthsAt(D, Now())
|*CalculateAgeInWeeksAt(D, X)* |Determines the age of a person with birthdate D in weeks as of the date/time X
|*CalculateAgeInWeeks(D)* |Determines the age of a person with birthdate D in weeks as of today. Equivalent to CalculateAgeInWeeksAt(D, Now())
|*CalculateAgeInDaysAt(D, X)* |Determines the age of a person with birthdate D in days as of the datetime X
|*CalculateAgeInDays(D)* |Determines the age of a person with birthdate D in days as of now. Equivalent to CalculateAgeInDaysAt(D, Now())
