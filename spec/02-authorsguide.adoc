...
The Boolean type in CQL supports the logical values [.kw]#true# and [.kw]#false#. These values are most often encountered as the result of Comparison Operators, and can be combined with other boolean-valued expressions using Logical Operators. Note that CQL supports three-valued logic, see the section on <<Missing Information>> for more.
...
|*~* |Equivalent |Returns true if the arguments are the same value, or are both unknown. Returns null if either or both arguments are null
...
Comparison, like equality, when no precision is specified, is performed to the unit with the most precision of either input. However, each operator allows a precision specifier to be used. For example:
...
|*GeometricMean*| Return the geometric mean of the non-null elements in the llist
