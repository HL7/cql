...
In addition, any operation involving [.id]#Decimals# can be used with values of type [.id]#Integer# because [.id]#Integer# values can always be implicitly converted to [.id]#Decimal# values.
...
If the [.id]#onsetDateTime# is not present, the result of this expression is [.kw]#null#. Furthermore, nulls will in general _propagate_, meaning that if the result of [.id]#FirstPharyngitis# is [.kw]#null#, the result of accessing the [.id]#onsetDateTime# element is also [.kw]#null#.
...
This expression results in an interval that contains the [.id]#Integers# 3 and 4, but not 5.
...
* For [.id]#Strings#, equivalence ignores case, locale, and whitespace.
* For [.id]#Decimals#, equivalence means the values are the same to the precision of the least precise value being compared.
...
* For [.id]#Datetimes#, [.id]#Dates#, and [.id]#Times#, equivalence is the same as equality except it will return [.kw]#false# instead of [.kw]#null# if the values being compared have differing precisions.
